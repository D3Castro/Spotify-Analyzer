...
from flask import Blueprint, g, request, session, json, jsonify, make_response, current_app
...
def get_user_profile(token):
    ...
    profile_response = requests.get(user_profile_api_endpoint, headers=get_auth_header(token))
...
def create_user(data):
    ...
    if db.execute('SELECT spotify_id FROM users WHERE spotify_id = ?', (data['id'],)).fetchone() is None:
        db.execute(
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.user = get_db().execute('SELECT * FROM users WHERE spotify_id = ?', (user_id,)).fetchone()
